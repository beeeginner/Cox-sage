...
class MyOwnDataset:
    ...
    def process(self):
        ...
        X = np.log2(X + 1)  # Log normalization of the gene expression
...
def loss_function(thetas, survival_time, events):
    ...
    Events_index = torch.nonzero(events == 1).squeeze()  # Find where events have occurred (i.e., death)
    for i in Events_index:
        loss += thetas[i]
        risk_sum = torch.sum(torch.exp(thetas[survival_time >= survival_time[i]]))  # Compute risk for all patients
...
def c_index(risk_pred, survival_time, events):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            # Compare predicted risks with actual survival times for evaluation
...
def train(gene, adj_list, clinical, num_layers, save_path, log, seed=0, lr=1e-5, wd=1e-3):
    ...
    np.random.seed(seed)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dataset = MyOwnDataset(node_csv=gene, adj_list=adj_list, label_csv=clinical, device=device)
    ...
    class SageCox(nn.Module):
        ...
        def __init__(self, num_layers=4):
            ...
            # Define layers based on the number of layers required
            if num_layers == 4:
    ...
    train_samples, test_samples = train_test_split(all_samples, train_size=0.8, random_state=seed, stratify=data.vital_status.cpu())
    ...
    val_samples = stratified_random_partition(train_samples, partitions=k, stratify_labels=data.vital_status.cpu()[train_mask], random_state=seed)
    ...
    for idx, val in enumerate(val_samples):
        ...
        for epoch in range(epochs):
            if counter > early_stopping_patience:
                print(f'Early stopping triggered! Finally epoch:{epoch}')
                break
            model.train()
            optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                train_index, val_index = c_index(out[train_mask], data.survival_time[train_mask], data.vital_status[train_mask]), c_index(out[val_mask], data.survival_time[val_mask], data.vital_status[val_mask])
